...
def in_tree(node, tree):  # XXX TODO
    """
    Checks if a node is inside a tree. 
    Inputs: node (Node?) and tree (Tree?)
    Outputs: boolean
    -Ben
    """
    if not tree:
...
def cycle_check(puta_end, start, graph):  # XXX use the flat_tree trick!
    ...
    """
    This function checks if the graph has a cycle. It starts with start, and 
    Inputs:
    Output: boolean
    -Ben
    """
    visited = []    # visited keeps track of all the nodes that have been visited already -Ben
    def inner(next_):
        if next_ not in graph:  # since these aren't actually graphs and some nodes dont have parents
            return False
        elif not graph[next_]:  # next_ has no connections to make, therefore can't have a cycle -Ben
            return False
        elif puta_end in graph[next_]:   # what's puta_end? -Ben
            return True
...
class TreeNode(defaultdict):  # FIXME need to factory this to allow separate trees!
    ...
    pad = '  '    # TreeNode...holds no data? -Ben
    #prefix = []
    #existing = {}  # FIXME CAREFUL WITH THIS
    #current_parent = None

    def print_tree(self, level = 0):    # for the sake of collapsing stuff, I don't think I need to worry about this function -Ben
        output = ''
...
def newTree(name, **kwargs):    # This function appears to be unfinished. Don't worry about it, I suppose -Ben
    ...
    return Tree, newTreeNode    # A function (Tree) and type? (newTreeNode) -Ben

def creatTree(root, relationshipType, direction, depth, url_base='matrix.neuinfo.org:9000', json=None):
    """
    Make a new tree based on the given parameters
    Inputs: root, relationshipType, direction, depth, url_base (all Strings), json (Json)
    Outputs: named_hierarchy (Tree)
    -Ben
    """
    ...
    nodes = {n['id']:n['lbl'] for n in j['nodes']}    # nodes is a dictionary, with id as keys and 1b1 as values. These are from j['nodes']. What do these mean? -Ben
    nodes[CYCLE] = CYCLE  # make sure we can look up the cycle                                            # Adding another entry to nodes. Why? -Ben
    edgerep = ['{} {} {}'.format(nodes[e['sub']], e['pred'], nodes[e['obj']]) for e in j['edges']]    # A list of three things. Don't worry too much about this; it's not used elsewhere in the code, and only shows up in the return statement -Ben

    # This set of lines makes objects and parents. Both are dictionaries, with lists as values. -Ben
    ...
    # names and pnames are both dictionaries. One is based on objects, the other parents. -Ben
    names = {nodes[k]:[nodes[s] for s in v] for k,v in objects.items()}
    pnames = {nodes[k]:[nodes[s] for s in v] for k,v in parents.items()}

    # The rest of the code is just to make named_hierarchy
    Tree, _ = newTree(query, parent_dict=parents)

    def build_tree(obj, existing = {}, flat_tree = set()):
        """
        Builds a tree using obj as the root. obj is intended to be root. 
        Inputs: obj (), existing (dictionary), and flat_tree (set)
        Outputs: t, existing (dictionary that is modified with this function)
        -Ben
        """
        subjects = objects[obj]
        t = Tree()    # Make new tree? -Ben
        t[obj]        # This line doesn't really do anything. It returns a subset from t, but doesn't assign it to anything or do anything to it -Ben
...
def levels(tree, p, l = 0):    # This function is only used in main. This is probably an important function, as it can't be a helper function -Ben
    """
    Input: tree (Tree), p (int?) (number of parents?), l (int?) (levels?)
    Output: list of  
    """
    if p == 0:
...
def count(tree): 
    """
    This function adds up the number of nodes in the tree. Public function, since it only appears in main and, thus, isn't a helper function
    Input: tree (Tree)
    Output: sum of nodes (int)
    -Ben
    """
    return sum([count(tree[k]) if tree[k] else 1 for k in tree])

def todict(tree):
    """
    This function makes a dictionary out of a tree. 
    Input: tree (Tree)
    Output: dictionary
    -Ben
    """
    return {k:todict(v) for k, v in tree.items()}

def flatten(tree, out=[]): 
    """
    This function takes the tree and flattens it down to a list. The first entry is the root name. 
    Note that the output list is depth-based. 
    Inputs: tree (Tree) and out (list)
    Output: out (list)
    -Ben
    """
    for name, subtree in tree.items():    # name is the keys to the tree, and subtree is all the values, also trees
        out.append(name)
        flatten(subtree, out)
    return out

def inv_edges(json):     # It's never used in this file -Ben
